...
export default function FilterableResults({
  ...
  */
  function updateFilterOptionToState(collectionIndex, optionIndex) {
    const updatedFilterOptions = updateFilterOption(
      filterOptions, collectionIndex, optionIndex
    );
  ...
  */
  function resetFilterCollectionToState(collectionIndex) {
    const updatedFilterOptions = resetFilterCollection(filterOptions, collectionIndex);
    resetFeedFromFilterUpdate(updatedFilterOptions);
  }
  /**
  ...
  */
  function resetThenSetFilterCollectionToState(collectionIndex, optionIndex) {
    const resetFilterOptions = resetFilterCollection(filterOptions, collectionIndex);
    const updatedFilterOptions = updateFilterOption(
      resetFilterOptions, collectionIndex, optionIndex
    );
    resetFeedFromFilterUpdate(updatedFilterOptions);
  }
  const tabIDToIdx = new Map(filterOptions[DEFAULT_TAB_IDX].options.map((opt, i) => [opt.data.id, i]));
  const setTab = (tabID) => {
    if (tabID === DEFAULT_TAB_ID) {
      return resetFilterCollectionToState(DEFAULT_TAB_IDX);
    }
    return resetThenSetFilterCollectionToState(DEFAULT_TAB_IDX, tabIDToIdx.get(tabID));
  };
  return (
    <>
      <div className="Sider">
      ...
      <div className="Content">
        {useTabs && <Tabs tabFilter={filterOptions[DEFAULT_TAB_IDX]} setTabFilter={setTab}/>}
        <Results
          results={results}
          hasMore={hasMore}
          fetchMore={fetchMore}
...
function Tabs({tabFilter, setTabFilter}) {
  ...
  tabs.unshift(
    <button onClick={() => setTabFilter(DEFAULT_TAB_ID)}>
      Most Relevant
    </button>
  );
  return (
...
export function getTabFromTypeFilterCollection(filterCollection) {
  const enabledTypes = filterCollection.options.filter((option) => option.enabled);
